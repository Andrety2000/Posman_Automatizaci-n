...
Feature: Apis de Netflix para consumo masivo
  ...
  @t7
  Scenario: Ingrese el Nombre de la plataforma //Ejercicio 1
    Given que la application esta operativa
    When obtiene al cliente: <id> suscrito
    And ejecuta la creacion "titulo"
    And inserto los valores del cliente con nombre "Pc" y trabajo "Shooter"
    Then valida que la respuesta es 200
    And valida el "platform" sea "pc"
    And valida el "category" sea "shooter"


  @t7
  Scenario: Ingresar el Nombre caso error //Ejercicio 2
    Given que la aplicación esta operativa
    And valida que el trabajo es "Caso error"
    Then valida que la respuesta es 400
