...
public class NetflixSD {
    ...
    public void queLaApplicationEstaOperativa() {
        String titulo = lastResponse().jsonPath().param("", 0).getString("title");
        System.out.println(titulo);
    }

    @When("obtiene al cliente: <id> suscrito")
    public void obtieneAlClienteIdSuscrito() {
        String titulo = lastResponse().jsonPath().param("", 0).getString("title");
        System.out.println(titulo);
    }

    @And("ejecuta la creation {string}")
    public void ejecutaLaCreation(String arg0) {
        String titulo = lastResponse().jsonPath().param("", 0).getString("title");
        System.out.println(titulo);
    }
}
